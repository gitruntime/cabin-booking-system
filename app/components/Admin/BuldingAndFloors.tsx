...
export default function BuldingAndFloors() {
  ...
  useEffect(() => {
    const fetchBuildings = async () => {
      ...
      try {
        ...
        console.log("🚀 ~ fetchBuildings ~ res:", res)
        const data: any[] = res?.data ?? [];
        const mapped: BuildingType[] = data.map((b: any) => ({
          id: b._id ?? b.id,
          name: b.name,
          floors: [],
        }));
        const allFloors: FloorType[] = data.flatMap((b: any) =>
          (b.floors ?? []).map((f: any) => ({
            id: f._id ?? f.id,
            buildingId: b._id ?? b.id,
            name: f.name,
            level: f.level ?? 0,
          }))
        );
        setBuildings(mapped);
        setFloors(allFloors);
        if (mapped.length > 0) setSelectedBuilding(mapped[0]);
      } catch (err) {
  ...
  const handleBuildingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingBuilding) {
        ...
        if (buildings.length === 0) setSelectedBuilding(newB);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200/60 dark:bg-slate-900 dark:border-slate-800/80 shadow-xs space-y-4">
          ...
          <div className="space-y-2">
            {loadingBuildings ? (
              <div className="p-6 text-center text-xs text-slate-400 dark:text-slate-500">Loading buildings...</div>
            ) : buildings.length === 0 ? (
